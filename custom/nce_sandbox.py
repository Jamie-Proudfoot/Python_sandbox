...
def NCE_verbose(Z,Y):
    ...
    Kz = int(np.max(Z) + 1)
    N = len(Z)
    # Joint probability of target and source labels P(z,y) (Kz*Ky matrix)
    joint = np.histogram2d(Z, Y, bins = (Kz, Ky))[0] / N
    # Marginal probability of source label P(z) (Kz*1 matrix)
    marginal = joint.sum(axis=1, keepdims=True)
    # Conditional probability matrix P(y|z) = P(z,y) / P(z) (Kz*Ky matrix)
    conditional = np.divide(joint,marginal)
    # Mask variable for boolean where P(z) is not zero (Kz boolean vector)
    mask = marginal.reshape(-1) != 0
    # Remove zero values to avoid log asymptote
    conditional = conditional[mask] + 1e-16
    # Entropy of Y given Z == H(y|z) (Kz*1 matrix)
